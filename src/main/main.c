...
static void resolveSaveIdForPath(const char* rom_path, char* out_save_id, size_t out_save_id_len) {
	...
	if (!out_save_id || out_save_id_len == 0) return;
	out_save_id[0] = '\0';
	if (!rom_path || !rom_path[0]) return;

	getEmuName(rom_path, core_registry_name);
	if (CORE_REGISTRY_resolveSaveIdForSystem(core_registry_name, out_save_id, out_save_id_len) == 0) return;
...
static int hasRoms(char* dir_name) {
	...
	if (!hasEmu(core_registry_name)) return has;
...
static void addEntries(Array* entries, char* path) {
	...
	if (dh!=NULL) {
		...
		while((dp = readdir(dh)) != NULL) {
			...
			if (prefixMatch(COLLECTIONS_PATH, full_path) && dp->d_type != DT_DIR) type = ENTRY_DIR;
...
static void openRom(char* path, char* last) {
	...
	if (should_resume) {
		...
		if (has_m3u) {
			...
			snprintf(disc_path_path, sizeof(disc_path_path), "%s/%s/%s/slot %d/disc.txt", SHARED_USERDATA_PATH, save_id, rom_file, atoi(slot) + 1);
			...
			if (current_screen == SCREEN_SETTINGS) {
				...
				if (total>0) {
					...
					for (int i=top->start,j=0; i<top->end; i++,j++) {
						...
						UI_LIST_initRow(&browser_rows[j],
							UI_ROW_KIND_BROWSER, entry_name,
							entry_unique ? entry_unique : "");
						browser_rows[j].clip_right =
							(had_thumb && j!=selected_row) ? ox :
							screen->w;
					}
				}
				...
				if (can_resume) {
					...
					browser_view.top_group.hints[0].label =
						BTN_SLEEP==BTN_POWER||simple_mode ?
						UI_HINT_LABEL_NONE :
						UI_HINT_LABEL_SETTINGS;
					browser_view.top_group.hints[0].text =
						BTN_SLEEP==BTN_POWER||simple_mode ?
						"SLEEP" : NULL;
				}
				...
				if (total==0) {
					if (stack->count>1) {
						...
						browser_view.bottom_group.hints[0].button =
							UI_BUTTON_ID_B;
						browser_view.bottom_group.hints[0].label =
							UI_HINT_LABEL_BACK;
					}
				}
					...
					browser_view.bottom_group.hints[0].label =
						UI_HINT_LABEL_BACK;
					browser_view.bottom_group.hints[1].button =
						UI_BUTTON_ID_A;
					browser_view.bottom_group.hints[1].label =
						UI_HINT_LABEL_OPEN;
				}
				else {
					browser_view.bottom_group.count = 1;
